...
def create_loss_fn(t_array, y_array, static):
    ...
    def objective_jax(decision_vars_scaled):
        ...
        alpha_s, beta_s, b_s, theta_s, f0_s= decision_vars_scaled
...
alertar_telegram("Iniciando Otimização por Evolução Diferencial (Escalonada)")

# Limites ESCALONADOS. O otimizador vai procurar os multiplicadores.
b_alpha_s = (0.5, 1.2)  # x 1.0  -> Físico: (0.5, 1.2)
b_beta_s = (0.1, 100.0)  # x 1e-8 -> Físico: (1e-9, 1e-6)
b_b_s = (0.1, 5.0)  # x 1e-3 -> Físico: (1e-4, 5e-3)
b_theta_s = (0.01, 1.0)  # x 1.0  -> Físico: (0.01, 1.0)
b_f0_s = (0.1,100)
param_bounds_scaled = [b_alpha_s, b_beta_s, b_b_s, b_theta_s,b_f0_s]

print("\n--- Iniciando Evolução Diferencial (Com Escalonamento) ---")
result = differential_evolution(
    func=lambda x: obj_for_scipy(x)[0],
    bounds=param_bounds_scaled,  # Passamos os limites escalonados
    strategy='best1bin',
    maxiter=1000,  # Você pode reduzir isso agora que o problema está bem condicionado
    popsize=20,  # 100 é um pouco de exagero e demora muito; 20-30 costuma bastar
    tol=1e-6,
    disp=True,
    updating='deferred',
    workers=1
...
alpha_opt_s, beta_opt_s, b_opt_s, theta_opt_s, f0_opt_s = result.x

alpha_opt = alpha_opt_s * 1.0
beta_opt = beta_opt_s * 1e-8
b_opt = b_opt_s * 1e-3
theta_opt = theta_opt_s * 1.0
f0_opt = f0_opt_s*1e-4
